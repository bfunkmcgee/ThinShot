...
"""Find corpses that are still standing up.
    ...
    python tools/check_dead_stance.py <unit_root> [--fix]
...
def main() -> None:
    ...
    death_anim = root / "Idle" / "animations" / "standing_idle_to_dead"
    if not dead.is_dir():
    ...
    print("\n%d direction(s) still standing: %s" % (len(standing), ", ".join(standing)))
    if not fix:
