...
const BLOOD_SCOUT := Color("7a2b1e")
const BLOOD_GOBLIN := Color("3b4a2e")
...
## Impact burst: debris sprays back toward the shooter, plus sand motes.
func impact(pos: Vector2, dir: Vector2, team: int, lethal := false) -> void:
	var gore := BLOOD_SCOUT if team == 0 else BLOOD_GOBLIN
	_add(pos, Vector2.ZERO, 0.22, 4.0, 16.0, Color(SPARK_WARM, 0.7), Shape.RING)
	var count := 14 if lethal else 10
	for i in count:
		_add(pos, _spread(-dir, deg_to_rad(50), _rng.randf_range(40, 150)),
				0.4, _rng.randf_range(2, 4), 1.0,
				Color(gore, 0.9), Shape.PIXEL, 420.0, 1.0)
	for i in 4:
...
## Permanent ground mark left where a unit fell.
func stain(pos: Vector2) -> void:
	if _marks.size() >= MAX_MARKS:
		_marks.pop_front()
	_marks.append({"pos": pos, "size": 14.0, "col": STAIN_COLOR})
	queue_redraw()
